...
*)
	echo -e " 
	此 ${red}脚本${none} 不支持你的系统。 ${yellow} ${none}
...
_load() {
	local _dir="/etc/v2ray/adam/v2ray/src/"
	. "${_dir}$@"
}
_sys_timezone() {
	IS_OPENVZ=
...
path_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认: [${cyan}bypass$none]):")" path
		[[ -z $path ]] && path="bypass"
...
proxy_site_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认: [${cyan}https://qatarairways.com$none]):")" proxy_site
		[[ -z $proxy_site ]] && proxy_site="https://qatarairways.com"
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}adamz.cc$none)"): " sspass
		[ -z "$sspass" ] && sspass="adamz.cc"
		case $sspass in
		*[/$]*)
			echo
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		if [[ ! -d $(pwd)/config ]]; then
			...
			echo
			echo -e " 请确保你有完整的上传 adamz.cc 的 V2Ray 一键安装脚本 & 管理脚本到当前 ${green}$(pwd) $none目录下"
			echo
			exit 1
		fi
		mkdir -p /etc/v2ray/adam/v2ray
		cp -rf $(pwd)/* /etc/v2ray/adam/v2ray
	else
		pushd /tmp
		git clone https://github.com/CheungKwokHo/v2ray -b "$_gitbranch" /etc/v2ray/adam/v2ray --depth=1
		popd

	fi

	if [[ ! -d /etc/v2ray/adam/v2ray ]]; then
		echo
...
config() {
	cp -f /etc/v2ray/adam/v2ray/config/backup.conf $backup
...
backup_config() {
	sed -i "18s/=1/=$v2ray_transport/; 21s/=34443/=$v2ray_port/; 24s/=$old_id/=$uuid/" $backup
	if [[ $v2ray_transport -ge 18 ]]; then
		sed -i "30s/=10000/=$v2ray_dynamic_port_start_input/; 33s/=20000/=$v2ray_dynamic_port_end_input/" $backup
	fi
	if [[ $shadowsocks ]]; then
		sed -i "42s/=/=true/; 45s/=6666/=$ssport/; 48s/=adamz.cc/=$sspass/; 51s/=chacha20-ietf/=$ssciphers/" $backup
	fi
	[[ $v2ray_transport == [45] ]] && sed -i "36s/=adamz.cc/=$domain/" $backup
	[[ $caddy ]] && sed -i "39s/=/=true/" $backup
	[[ $ban_ad ]] && sed -i "54s/=/=true/" $backup
	if [[ $is_path ]]; then
		sed -i "57s/=/=true/; 60s/=adam/=$path/" $backup
		sed -i "63s#=https://qatarairways.com#=$proxy_site#" $backup
	fi
}
...
*)
	echo
	echo -e " 未知参数 <$red $args $none> ..."
	echo
	echo -e " 此脚本仅支持输入$green local / online $none参数"
	echo
