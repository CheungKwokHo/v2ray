...
socks_user_config() {
	echo
	while :; do
		read -p "$(echo -e "请输入$yellow用户名$none...(默认用户名: ${cyan}adam$none)"): " new_socks_username
		[ -z "$new_socks_username" ] && new_socks_username="adam"
		case $new_socks_username in
		*[/$]* | *\&*)
			echo
			echo -e " 用户名不能包含$red / $none或$red $ $none或$red & $none这三个符号.... "
			echo
			error
			;;
		*)
...
socks_pass_config() {
	echo
	while :; do
		read -p "$(echo -e "请输入$yellow密码$none...(默认密码: ${cyan}adamz.cc$none)"): " new_socks_userpass
		[ -z "$new_socks_userpass" ] && new_socks_userpass="adamz.cc"
		case $new_socks_userpass in
		*[/$]* | *\&*)
			echo
			echo -e " 密码不能包含$red / $none或$red $ $none或$red & $none这三个符号.... "
			echo
			error
			;;
		*)
