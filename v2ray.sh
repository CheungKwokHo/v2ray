...
backup="/etc/v2ray/adam_v2ray_backup.conf"

if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/adam/v2ray ]]; then

	. $backup

elif [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f /etc/v2ray/adam_v2ray_backup.txt && -d /etc/v2ray/adam/v2ray ]]; then

	. /etc/v2ray/adam/v2ray/tools/v1xx_to_v3xx.sh

else
	echo -e "  ${red}出错了...请重新安装V2Ray${none} ${yellow} ${none}" && exit 1
fi

if [[ $mark != "v3" ]]; then
	. /etc/v2ray/adam/v2ray/tools/v3.sh
fi
...
v2ray_client_config="/etc/v2ray/adam_v2ray_config.json"
...
. /etc/v2ray/adam/v2ray/src/init.sh
systemd=true
# _test=true
...
shadowsocks_config() {
	echo
	echo
	echo -e " $red没有配置 Shadowsocks $none..."
	echo
	echo
...
shadowsocks_port_config() {
	...
	while :; do
		...
		$v2ray_port)
			echo
			echo -e " 不能和$cyan V2Ray 端口 $none一样...."
			echo
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}adamz.cc$none)"): " new_sspass
		[ -z "$new_sspass" ] && new_sspass="adamz.cc"
		case $new_sspass in
		*[/$]*)
			echo
			echo -e " 密码不能包含$red / $none或$red $ $none这两个符号.... "
			echo
			error
			;;
		*)
...
disable_shadowsocks() {
	...
	while :; do
		...
		if [[ "$y_n" == [Yy] ]]; then
			...
			echo
			echo
			echo -e "$green Shadowsocks 已关闭...$none"
			echo
			break
		elif [[ "$y_n" == [Nn] ]]; then
			echo
	...
	while :; do
		...
		if [[ -z $_opt ]]; then
			...
			[aA][Ii][aA][Ii] | [Dd][Dd])
				...
				break
				;;
			[Dd] | [Aa][Ii] | 64 | 64[Bb][Ll][Oo][Gg] | 64[Bb][Ll][Oo][Gg].[Cc][Oo][Mm] | 64[Bb][Oo][Yy] | [Aa][Ll][Tt][Ee][Rr][Ii][Dd])
...
path_config() {
	echo
	while :; do
		echo -e "请输入想要 ${magenta}用来分流的路径$none , 例如 /bypass , 那么只需要输入 bypass 即可"
		read -p "$(echo -e "(默认: [${cyan}bypass$none]):")" new_path
		[[ -z $new_path ]] && new_path="bypass"
...
proxy_site_config() {
	echo
	while :; do
		echo -e "请输入 ${magenta}一个正确的$none ${cyan}网址$none 用来作为 ${cyan}网站的伪装$none , 例如 https://qatarairways.com"
		echo -e "举例...假设你当前的域名是$green $domain $none, 伪装的网址的是 https://qatarairways.com"
		echo -e "然后打开你的域名时候...显示出来的内容就是来自 https://qatarairways.com 的内容"
		echo -e "其实就是一个反代..."
		echo -e "如果不能伪装成功...可以使用 v2ray config 修改伪装的网址"
		read -p "$(echo -e "(默认: [${cyan}https://qatarairways.com$none]):")" new_proxy_site
		[[ -z $new_proxy_site ]] && new_proxy_site="https://qatarairways.com"
...
change_v2ray_dynamicport() {
	if [[ $v2ray_transport -ge 18 ]]; then
		...
		# clear
		echo
		echo -e "$green 动态端口修改成功...不需要修改 V2Ray 客户端配置...保持原有的配置即可...$none"
		echo
	else
		echo
		echo -e "$red ...当前传输协议没有启用动态端口...$none"
		echo
		while :; do
...
change_proxy_site_config() {
	if [[ $v2ray_transport == [45] ]] && [[ $caddy && $is_path ]]; then
		...
		while :; do
			echo -e "请输入 ${magenta}一个正确的$none ${cyan}网址$none 用来作为 ${cyan}网站的伪装$none , 例如 https://qatarairways.com"
			echo -e "举例...你当前的域名是$green $domain $none, 伪装的网址的是 https://qatarairways.com"
			echo -e "然后打开你的域名时候...显示出来的内容就是来自 https://qatarairways.com 的内容"
			echo -e "其实就是一个反代..."
			echo -e "如果不能伪装成功...可以使用 v2ray config 修改伪装的网址"
		...
		echo " 修改成功..."
		echo
		echo -e " 打开域名 ${cyan}https://${domain}$none 检查"
		echo
		echo
	elif [[ $v2ray_transport == [45] ]] && [[ $caddy ]]; then
		path_config_ask
...
custom_uuid() {
	...
	while :; do
		...
		*[/$]* | *\&*)
			echo
			echo -e "  UUID 不能包含$red / $none或$red $ $none或$red & $none这三个符号.... "
			echo
			error
			;;
		*)
			...
			echo
			echo -e " 如果 UUID 格式不正确.. V2Ray 会失效...使用$cyan v2ray reuuid$none 生效"
			echo "----------------------------------------------------------------"
			echo
			pause
			uuid=$myuuid
...
get_v2ray_config() {
	...
	echo
	while :; do
		read -p "$(echo -e "正在使用 Xshell [${magenta}Y$none]:")" is_xshell
		if [[ -z $is_xshell ]]; then
			error
		else
			if [[ $is_xshell == [yY] ]]; then
				...
				echo
				# sz /etc/v2ray/adam_v2ray.zip
				local tmpfile="/tmp/2adam_v2ray_config_$RANDOM.json"
				...
				echo
				# echo -e "$yellow 解压密码 = ${cyan}adamz.cc$none"
				# echo
...
get_v2ray_config_info_link() {
	echo
	echo -e "$green 正在生成链接.... 稍等....$none"
	echo
	create_v2ray_config_text >/tmp/adam_v2ray.txt
	local random=$(echo $RANDOM-$RANDOM-$RANDOM | base64 -w 0)
	local link=$(curl -s --upload-file /tmp/adam_v2ray.txt "https://transfer.sh/${random}_adam_v2ray.txt")
	if [[ $link ]]; then
		echo
		echo "---------- V2Ray 配置信息链接-------------"
		echo
...
install_bbr() {
	...
	if [[ $test1 == "net.ipv4.tcp_congestion_control = bbr" && $test2 == "net.core.default_qdisc = fq" ]]; then
		echo
		echo -e "$green BBR 已经启用...无需再安装$none"
		echo
	else
		_load bbr.sh
		_try_enable_bbr
...
update_v2ray.sh() {
	if [[ $_test ]]; then
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/CheungKwokHo/v2ray/test/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	else
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/CheungKwokHo/v2ray/master/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	fi
	...
	if [[ $latest_version == $_version ]]; then
		echo
		echo -e "$green 没有发现新版本 $none"
		echo
	else
		echo
		echo -e " $green 发现新版本....正在更新.......$none"
		echo
		cd /etc/v2ray/adam/v2ray
		git pull
		cp -f /etc/v2ray/adam/v2ray/v2ray.sh $_v2ray_sh
		chmod +x $_v2ray_sh
		echo
		echo -e "$green 更新成功...当前 V2Ray 管理脚本 版本: ${cyan}$latest_version$none"
		echo
	fi
...
menu() {
	...
	while :; do
		...
		echo
		echo -e "温馨提示...如果不想执行选项...按$yellow Ctrl + C $none即可退出"
		echo
...
reinstall)
	...
	if [[ $is_uninstall_v2ray ]]; then
		cd
		cd - >/dev/null 2>&1
		bash <(curl -s -L https://raw.githubusercontent.com/CheungKwokHo/v2ray/v2ray.sh)
	fi
	;;
[aA][Ii] | [Dd])
	change_v2ray_alterId
